...
- **TenantCreditBalance** — authoritative per-tenant credit counter (`tenant_credit_balances`, UQ on tenant_id; migration `0003`). Columns: `period_start`, `allowance_credits` (monthly grant, resets on the 1st, no rollover), `topup_credits` (purchased; cached sum of live `credit_topup_lots`, spent only after the allowance), `period_spent`, `lifetime_spent`, `lifetime_purchased`. Locked with `with_for_update()` during `spend()` — this is the ELR-009b fix for the sum-then-check race. A tenant's first row is seeded with the plan allowance MINUS the ledger's month-to-date, so shipping this doesn't grant a free refill.
- **CreditTopupLot** — one row per top-up purchase (`credit_topup_lots`, migration `0005`, 2026-09-25). Columns: `tenant_id` (FK CASCADE), `credits_purchased`, `credits_remaining`, `purchased_at`, `expires_at` (purchase + `CREDIT_TOPUP_VALIDITY_DAYS`, default 365), `expired_at` (set by the expiry sweep), `reference_id` (Stripe session). Index `(tenant_id, expires_at)`. Only mutated while the tenant's `TenantCreditBalance` row is locked; spends drain soonest-expiring lots first.
