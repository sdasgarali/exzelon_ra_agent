...
- `send_system_email(db, tenant_id, to, subject, html, attachments=None, reply_to=None)` — the tenant-aware transactional sender for ALL system mail. Now provider-backed (`adapters/transactional/`): **Resend** or SMTP per `SYSTEM_MAIL_PROVIDER`, with a tenant's own SMTP always winning (deal notifications, email verification, password resets). Resolves a **per-tenant** sender from `tenant_settings` (keys `notification_sender_email`/`_name`, `notification_smtp_host`/`_port`/`_user`, `notification_smtp_password_enc` (Fernet), `notification_smtp_security` = starttls|ssl); falls back to global `settings.SMTP_*`; skips (returns False) if neither is set. `_smtp_send()` handles STARTTLS (587) and implicit SSL (465). `send_test_email()` powers the Settings "Send test" button. Configured via `PUT /settings/notifications/sender`.
...
1. **Lead Sourcing** — fetch jobs from boards, normalize, 3-layer deduplicate (external_job_id -> employer_linkedin -> company+title+state+city), sub-source tracking. Supports configurable per-adapter tuning (`job_source_tuning` setting), adapter result limit (`pipeline_adapter_limit`), and thread pool size (`pipeline_max_workers`) — all managed via Settings → Source Tuning tab. Exclusion keywords are pushed into upstream queries (JSearch/SerpAPI Google-syntax negatives, Adzuna `what_exclude`) when `push_exclusions_to_query` is on (default), so excluded postings are filtered server-side; the local `filter_excluded()` still runs as a backstop. **Company size/industry gate** (`_apply_company_gate`, helpers in `services/company_filters.py`) runs after dedup: drops confidential/blank employers, companies over `lead_sourcing_max_employee_count` (default 500), and excluded industries (IT/staffing/government, `lead_sourcing_excluded_industries`). Missing industry/size is filled by a bounded, cached Groq LLM step (`company_enrichment.resolve_company_metadata_batch`); unknown size/industry is never dropped. When `company_firmographic_provider=apollo`, that resolver first runs an authoritative **firmographic** lookup — Apollo `/organizations/enrich` **by domain** (`services/company_firmographics.py`) — to fill company **SIZE**, which the free LLM cannot determine reliably. It is paid (bounded by `company_firmographic_max_lookups`, cost category `firmographic`), needs a domain (name lookups return nothing), and is NEVER invoked on the `free_only` pre-enrichment gate path. Backfill existing companies/leads with `scripts/backfill_company_size.py` (Groq domain-guess → Apollo → persist to ClientInfo + leads). Drop reasons surface in run `counters_json` (`excluded_confidential`/`excluded_too_large`/`excluded_industry`/`enriched_companies`). Records per-source cost via `cost_tracker.record_pipeline_cost()`.
...
| Send Quota | `services/send_quota.py` | The SECOND meter — monthly email allowance, separate from credits because a send's marginal cost is ~$0. Counted from `OutreachEvent` (SENT/REPLIED/BOUNCED), not a counter, so no send path can drift. |
| Credit Metering | `services/credit_metering.py` | **Gate vs meter.** `check_credit_budget()` = pre-flight 402 gate (runs BEFORE paid work). `meter()`/`spend()` = record consumption (runs AFTER; NEVER raises — failing a pipeline after burning a paid API call would cost money and lose the record). `TenantCreditBalance` is the authoritative counter, locked with `SELECT ... FOR UPDATE` (ELR-009b — replaces the old sum-then-check race); `CreditUsage` remains the append-only audit ledger. Allowance is spent before top-ups; overage goes negative rather than clamping, so it stays visible. `spend()` RETRIES on contention (OperationalError/DBAPIError, 5 attempts, jittered backoff) and logs `credit_spend_lost` at ERROR if it finally gives up — swallowing a deadlock silently meant the busier a tenant got, the less we billed them. Monthly refill is lazy on access **and** swept by the `credit_refill` scheduler job (1st, 00:05). No rollover. Prices come from `core/credit_costs.py`. **Top-ups** (2026-09-25): $20 per 1,000 (`CREDIT_TOPUP_BLOCK_*`, deliberately dearer per credit than any plan), paid plans only; each purchase is a `CreditTopupLot` valid `CREDIT_TOPUP_VALIDITY_DAYS` (365). `grant_topup()` adds a lot; `spend()` first lapses the tenant's due lots, then drains soonest-expiry-first; `expire_topup_lots()` is swept daily by the `credit_topup_expiry` job (00:15 UTC). `/billing/usage` returns the offer (`topup`) and `credits.topup_next_expiry` so the UI never hardcodes a price. |
| Plan Change | `services/billing/plan_change.py` | **The only way a tenant's plan changes** — `change_plan(db, tenant, new_plan, reason, old_allowance=None)` sets `tenant.plan` and calls `credit_metering.adjust_allowance_for_plan_change()`, which shifts this month's remaining allowance by (new − old): upgrades get the difference immediately; downgrades are lowered but never below 0 and never deeper into existing overage; top-ups untouched; a stale month just refills on the new plan. Used by `subscription_service.upsert_from_stripe` (active/trialing → paid plan; **canceled/unpaid → Free**; past_due keeps the plan while Stripe retries; the end of a superseded subscription is ignored) and the super-admin tenant editor. Does not commit. |
...
| **Send Gate** | `services/send_gate.py` | Centralized send safety — `unified_send_gate()` runs 11 ordered checks. **Check 0 = monthly send quota** (`services/send_quota.py`) — runs FIRST because it is the only tenant-level check, so a failure makes every per-contact check below it wasted work; skipped for dry_run/reply; FAILS OPEN. All 4 send paths wired. Checks: contact status -> suppression -> email validation -> cooldown -> per-lead limit -> company cap -> sequence fatigue -> domain throttle -> AI orchestrator. **Test contacts** (`is_test=True`) skip checks 4-8 (cooldown, fatigue, caps) like replies do. |
