...
def create_refresh_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    """Create a JWT refresh token (long-lived, used to obtain new access tokens)."""
    to_encode = {"sub": data.get("sub"), "type": "refresh"}
    if expires_delta:
        expire = datetime.utcnow() + expires_delta
    else:
...
def create_password_reset_token(email: str, expires_delta: Optional[timedelta] = None) -> str:
    ...
    Args:
        ...
        expires_delta: Custom expiry. Defaults to 1 hour.
