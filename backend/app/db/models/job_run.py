...
# MySQL's TEXT caps at 64KB, so these JSON blobs need LONGTEXT there. Postgres TEXT
# and SQLite TEXT are already unbounded — `.with_variant` keeps one column definition
# portable across all three instead of branching on the dialect at import time.
_BIG_TEXT = LONGTEXT().with_variant(Text, "sqlite").with_variant(Text, "postgresql")
...
class JobRun(Base):
    ...
    counters_json = Column(_BIG_TEXT, nullable=True)  # JSON with inserted/updated/skipped counts
    lead_results_json = Column(_BIG_TEXT, nullable=True)  # JSON array of per-lead enrichment results
    ...
    summary_json = Column(_BIG_TEXT, nullable=True)  # Cached AI-generated summary report
