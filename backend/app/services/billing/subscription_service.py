...
#: Statuses on which the tenant holds the plan it paid for.
LIVE_STATUSES = (SubscriptionStatus.ACTIVE, SubscriptionStatus.TRIALING)
#: Terminal statuses — the subscription is over, the tenant returns to Free.
ENDED_STATUSES = (SubscriptionStatus.CANCELED, SubscriptionStatus.UNPAID)
...
def upsert_from_stripe(db, sub_obj: dict, tenant_id: int) -> SubscriptionRecord:
    ...
    if record is None:
        ...
        # An event for a subscription that is no longer this tenant's current one —
        # e.g. Stripe's late "deleted" for the sub they replaced. Letting it through
        # would overwrite the live record and downgrade a paying tenant.
        if (
            record is not None
            and stripe_sub_id
            and record.stripe_subscription_id
            and record.stripe_subscription_id != stripe_sub_id
            and _map_status(sub_obj.get("status", "incomplete")) in ENDED_STATUSES
        ):
            logger.info("Ignoring end of a superseded subscription", tenant_id=tenant_id,
                        stale_sub_id=stripe_sub_id, current_sub_id=record.stripe_subscription_id)
            return record
    if record is None:
    ...
    # Keep the tenant's plan tier in sync with the subscription. Every change goes
    # through change_plan() so the month's credit allowance moves with it.
    ...
    if tenant is not None:
        if record.status in LIVE_STATUSES and plan:
            try:
                target = TenantPlan(plan)
            except ValueError:
                target = None
            if target is not None and tenant.plan != target:
                change_plan(db, tenant, target, reason=f"stripe:{record.status.value}")
        elif record.status in ENDED_STATUSES and tenant.plan != TenantPlan.FREE:
            # Cancelled, or Stripe gave up collecting: back to Free. Nothing is deleted;
            # anything above Free's limits stays readable but can't grow. past_due is
            # NOT here — Stripe is still retrying, and ELR-023 handles suspension.
            change_plan(db, tenant, TenantPlan.FREE, reason=f"stripe:{record.status.value}")
