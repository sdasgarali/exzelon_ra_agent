...
class TestLeadService:
    def test_get_lead_stats(self, db_session):
        ...
        lead = LeadDetails(client_name="Test", job_title="Dev", lead_status=LeadStatus.NEW, posting_date=date.today())
        db_session.add(lead)
        db_session.commit()
    ...
    def test_bulk_archive_leads(self, db_session):
        ...
        lead = LeadDetails(client_name="Del", job_title="QA", lead_status=LeadStatus.NEW, posting_date=date.today())
        db_session.add(lead)
        db_session.commit()
    ...
    def test_bulk_unarchive_leads(self, db_session):
        ...
        lead = LeadDetails(client_name="Arc", job_title="PM", lead_status=LeadStatus.NEW, posting_date=date.today(), is_archived=True)
        db_session.add(lead)
        db_session.commit()
...
class TestContactService:
    def test_get_contact_stats(self, db_session):
        ...
        c = ContactDetails(first_name="A", last_name="B", email="stats@test.com", client_name="Stats Corp")
        db_session.add(c)
        db_session.commit()
    ...
    def test_find_duplicate_contacts(self, db_session):
        ...
        c1 = ContactDetails(first_name="X", last_name="Y", email="svcdup@test.com", client_name="A Corp")
        c2 = ContactDetails(first_name="X", last_name="Z", email="svcdup@test.com", client_name="B Corp")
...
class TestTemplateService:
    def test_preview_template(self, db_session):
        ...
        t = EmailTemplate(name="Svc Test", subject="Hello {{contact_first_name}}", body_html="<p>Hi</p>", body_text="Hi", status=TemplateStatus.INACTIVE)
        db_session.add(t)
        db_session.commit()
    ...
    def test_duplicate_template(self, db_session):
        ...
        t = EmailTemplate(name="Orig", subject="Sub", body_html="<p>Body</p>", body_text="Body", status=TemplateStatus.ACTIVE)
        db_session.add(t)
        db_session.commit()
